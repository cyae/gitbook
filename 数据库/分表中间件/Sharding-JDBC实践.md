  ...
  shardingsphere:
    datasource:
      ds-0:
        ...
        url: jdbc:mysql://47.94.6.5:3306/ds-0?useUnicode=true&characterEncoding=utf8&tinyInt1isBit=false&useSSL=false&serverTimezone=GMT
        username: root
    sharding:
...
先创建两个数据库  `ds-0`、`ds-1`，两个库中分别建表  `t_order_0`、`t_order_1`、`t_order_2` 、`t_order_item_0`、`t_order_item_1`、`t_order_item_2` 6 张表，下边实操看看如何在不同场景下应用  `sharding-jdbc`  的 4 种分片策略。
    ...
    sharding:
      tables:
        t_order:
          database-strategy:
            standard:
              precise-algorithm-class-name: com.xiaofu.sharding.algorithm.dbAlgorithm.MyDBPreciseShardingAlgorithm
    ...
    sharding:
      tables:
        t_order:
          table-strategy:
            standard:
              precise-algorithm-class-name: com.xiaofu.sharding.algorithm.tableAlgorithm.MyTablePreciseShardingAlgorithm
              sharding-column: order_id
```
        ...
        t_order:
          database-strategy:
            standard:
              precise-algorithm-class-name: com.xiaofu.sharding.algorithm.dbAlgorithm.MyDBPreciseShardingAlgorithm
              #  范围分片算法
              range-algorithm-class-name: com.xiaofu.sharding.algorithm.dbAlgorithm.MyDBRangeShardingAlgorithm
    ...
    sharding:
      tables:
        t_order:
          database-strategy:
            complex:
              algorithm-class-name: com.xiaofu.sharding.algorithm.dbAlgorithm.MyDBComplexKeysShardingAlgorithm
...
行表达式分片策略（`InlineShardingStrategy`），在配置中使用  `Groovy`  表达式，提供对 SQL 语句中的  `=`  和  `IN`  的分片操作支持，它只支持单分片健。
...
Hint 分片策略（`HintShardingStrategy`）相比于上面几种分片策略稍有不同，这种分片策略无需配置分片健，分片健值也不再从 SQL 中解析，而是由外部指定分片信息，让 SQL 在指定的分库、分表中执行。`ShardingSphere`  通过  `Hint` API 实现指定操作，实际上就是把分片规则`tablerule` 、`databaserule`由集中配置变成了个性化配置。
